...
async def main():
    ...
    print("---------------------------------xpath---------------------------------------")
    # 查找一组 span class=text 的元素，返回数组
    text_list = await page.xpath("//span[@class='text']")
    print("text list[%s]: %s" % (len(text_list), text_list))

    # 使用别名
    text_list = await page.Jx("//span[@class='text']")
    print("text list[%s]: %s\n" % (len(text_list), text_list))

    tag_list = await page.Jx("//div/div/a[@class='tag'][2]")
    print("tag list[%s]: %s" % (len(tag_list), tag_list))
    print("\n\n")
